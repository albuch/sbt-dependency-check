...
object DependencyCheckListSettingsTask {
  def logSettings(settings: Settings, failBuildOnCVSS: Float, formats: Seq[String], outputDirectory: String, scanSet: Seq[sbt.File],
    ...
    logStringSetting(NVD_API_KEY, "dependencyCheckNvdApiKey", log)
    logStringSetting(NVD_API_DELAY, "dependencyCheckNvdApiDelay", log)
    logStringSetting(NVD_API_MAX_RETRY_COUNT, "dependencyCheckNvdApiMaxRetryCount", log)
